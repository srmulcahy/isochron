...
#'   \code{\link{tnh}} model regression.  The output cofidence intervals can be
...
#'   \code{\link{ciplt}}
#' 
#' @param df Input data frame for results of \code{\link{tnh}} regression 
#' @param b Slope of the \code{\link{tnh}} regression through the data
#' @param nmad nMad of the \code{\link{tnh}} regression through the data
...
#' C <- tnhci(P$dat, P$coef$slope, P$fit$nmad, n = 300)
tnhci <- function(df, b, nmad, n = 999){
	require(plyr)
	lx <- length(df$e)
	...
	ls <- split(ys, col(ys))
	f <- function(ls){
	  tnh(df$x, unlist(ls), df$sx, df$sy, p = 2.5, r = 0)$coef$slope
	}
